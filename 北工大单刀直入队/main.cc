...
#include "behaviors/robanbehavior.h"
...
void PrintHelp()
{
    ...
    cout << " --rsg <rsg>\trsg file for the roban model." << endl;
...
string agentType("robanagent");
string rsg("rsg/agent/roban/roban.rsg");
void ReadOptions(int argc, char* argv[])
{
    ...
    for( int i = 0; i < argc; i++)
    {
        ...
        else if ( strncmp( argv[i], "--mhost", 7 ) == 0 )
        {
            ...
            rsg = "rsg/agent/roban/roban_hetero.rsg " + string(argv[i+1]);
...
{
    Behavior *behavior;
    if (agentType == "robanagent") {
        behavior = new RobanBehavior(teamName, uNum, namedParams, rsg);
    }
