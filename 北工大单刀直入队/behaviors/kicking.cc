 ...
 */
SkillType RobanBehavior::kickBall(const int kickTypeToUse, const VecPosition &target) {
    kickTarget = target;
...
/*Computes an estimated cost of reaching the appropriate position to execute a kick
  */
double RobanBehavior::computeKickCost(VecPosition target, SkillType kickType) {
 ...
 */
SkillType RobanBehavior::kickBallAtPresetTarget() {
...
void RobanBehavior::resetKickState() {
...
SkillType RobanBehavior::kickBallAtTargetSimplePositioning(const VecPosition &targetToKickAt, SkillType kick_skill, int kickType) {
...
double RobanBehavior::getParameter(const string& name) {
    return atof(namedParams.find(name)->second.c_str());
}

double RobanBehavior::getStdNameParameter(const SkillType kick_skill, const string& parameter) {
    string prefix = "";
    switch(kick_skill) {
...
void RobanBehavior::getSkillsForKickType(int kickType, SkillType skills[]) {
    switch(kickType) {
    case KICK_IK:
...
VecPosition RobanBehavior::navigateAroundBall(VecPosition target, double PROXIMITY_THRESH, double COLLISION_THRESHOLD) {
