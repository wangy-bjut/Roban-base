    ...
    rsg( rsg_ )
{
    ...
    //cout << "Constructing of Roban Behavior" << endl;

    srand ((unsigned)time(NULL) );
    srand48((unsigned)time(NULL));

    classname = "RobanBehavior"; //TODO: eliminate it...
...
string RobanBehavior::Think(const std::string& message) {

    //  cout << "(RobanBehavior) received message " << message << endl;
 ...
 * Throws string
 */
void RobanBehavior::readSkillsFromFile( const std::string& filename) {
...
void RobanBehavior::calculateAngles() {
...
void RobanBehavior::resetSkills() {
    skills[skill]->reset();
...
void RobanBehavior::resetScales() {
...
VecPosition RobanBehavior::collisionAvoidance(bool avoidTeammate, bool avoidOpponent, bool avoidBall, double PROXIMITY_THRESH, double COLLISION_THRESH, VecPosition target, bool fKeepDistance) {
    // Obstacle avoidance
...
VecPosition RobanBehavior::collisionAvoidanceCorrection(VecPosition start, double PROXIMITY_THRESH, double COLLISION_THRESH, VecPosition target, VecPosition obstacle) {
...
VecPosition RobanBehavior::collisionAvoidanceApproach(double PROXIMITY_THRESH, double COLLISION_THRESH, VecPosition target, VecPosition obstacle) {
    return collisionAvoidanceApproach(me, PROXIMITY_THRESH, COLLISION_THRESH, target, obstacle);
}

VecPosition RobanBehavior::collisionAvoidanceApproach(VecPosition start, double PROXIMITY_THRESH, double COLLISION_THRESH, VecPosition target, VecPosition obstacle) {
...
SkillType RobanBehavior::getWalk(const double& direction, const double& rotation, double speed, bool fAllowOver180Turn)
{
    return getWalk(WalkRequestBlock::PARAMS_DEFAULT, direction, rotation, speed, fAllowOver180Turn);
}

SkillType RobanBehavior::getWalk(WalkRequestBlock::ParamSet paramSet, const double& direction, double rotation, double speed, bool fAllowOver180Turn)
{
    ...
    // RobanBehavior::act() will use the speed components in velocity
...
SkillType RobanBehavior::goToTargetRelative(const VecPosition& targetLoc, const double& targetRot, const double speed, bool fAllowOver180Turn, WalkRequestBlock::ParamSet paramSet)
{
...
SkillType RobanBehavior::goToTarget(const VecPosition &target) {
...
double RobanBehavior::getLimitingAngleForward() {
...
void RobanBehavior::getTargetDistanceAndAngle(const VecPosition &target, double &distance, double &angle) {
...
bool RobanBehavior::beamablePlayMode() {
    int pm = worldModel->getPlayMode();
    return pm == PM_BEFORE_KICK_OFF || pm == PM_GOAL_LEFT || pm == PM_GOAL_RIGHT;
}

bool RobanBehavior::improperPlayMode() {
...
bool RobanBehavior::improperPlayMode(int pm) {
...
bool RobanBehavior::kickPlayMode() {
    return kickPlayMode(worldModel->getPlayMode());
}

bool RobanBehavior::kickPlayMode(int pm, bool eitherTeam) {
...
bool RobanBehavior::isIndirectKick() {
    return isIndirectKick(worldModel->getPlayMode());
}

bool RobanBehavior::isIndirectKick(int pm) {
...
string RobanBehavior::getMonMessage() {
    string ret = monMsg;
    monMsg = "";
    return ret;
}
